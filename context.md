...
- Stable-plane compensation is exact for the independently selected world plane: `f(d) = 35 × (d − z_s) / (6.5 − z_s)`, where `d` is camera position and `z_s` is the stable plane depth. At the initial plane `z_s = −1.9`, this gives 35 mm at the 6.5 m far position and 16.25 mm at the 2 m near position.
...
- The initial composition starts at `t = 0` with the camera at `6.5 m` and the sphere at `z = -1.9`, halfway in depth between the nearer purple cube (`z = 0.8`) and farther teal cube (`z = -4.6`). The independent stable-depth indicator also starts at the sphere center plane so the 35 mm opening frame compensates the sphere exactly; it remains independently draggable afterward. Both cubes begin in mirrored corner-forward orientations and remain user-movable.
- Compensation is exact pinhole-plane geometry: for camera position `d`, selected world-depth plane `z_s`, and any point offset `X` on that plane, `x_ndc = 2 f X / (sensorWidth (d - z_s))`. Choosing `f = k(d - z_s)` for a captured constant `k` cancels depth exactly, so all points on the selected plane retain identical image coordinates. The initial reference is `k = 35 / (6.5 - (-1.9)) = 35 / 8.4`. A volumetric sphere extends in front of and behind that plane, so its silhouette may change slightly from genuine perspective; do not counter-scale scene geometry to conceal that depth effect.
- Compensation preserves the captured `f / Z` ratio at the independently selected stable-depth plane. Both cubes can translate with one-button drag and rotate while both mouse buttons are held.
- The focal-length annotation is a progressive-disclosure control. Hover, keyboard focus, or tap reveals a compact 12–70 mm slider anchored above the camera body. Manual focal adjustment leaves camera position unchanged. When Dolly Zoom is active, changing focal length keeps it active and captures the new compensation ratio; when Dolly Zoom is off, it remains off. Keep the annotation stationary and do not add a permanent second control row.
- Enabling Dolly Zoom captures the current `f / (d − z_s)` ratio. It must not change the focal length at the moment the switch is enabled; only subsequent camera movement adjusts the lens. Moving the stable-depth plane while compensation is active likewise captures a new ratio without changing the current frame.
